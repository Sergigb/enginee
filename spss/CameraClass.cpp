...
void CameraClass::TranslateLocal(float x, float y, float z){
	...
	DirectX::XMFLOAT4X4 rotMat_;
	
	
	transMat = DirectX::XMMatrixTranslation(x, y, z);
	rotMat = DirectX::XMMatrixRotationRollPitchYaw(m_rotationX, m_rotationY, m_rotationZ);

	rotMat = DirectX::XMMatrixMultiply(transMat, rotMat);
	
	DirectX::XMStoreFloat4x4(&rotMat_, rotMat);

	m_positionX += rotMat_._41;
	m_positionY += rotMat_._42;
	m_positionZ += rotMat_._43;
}
